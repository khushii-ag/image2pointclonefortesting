...
class SPVCNN18(nn.Module):
    def __init__(self, **kwargs):
        super().__init__()

        cr = kwargs.get('cr', 1.0)
        cs = [64, 64, 128, 256, 512, 512, 256, 192, 192]
        ...
        self.ds = BasicConvolutionBlock(cs[0], cs[0], ks=2, stride=2, dilation=1)
        ...
        self.stage2 = nn.Sequential(
            ResidualBlock(cs[1], cs[2], ks=3, stride=2, dilation=1),
            ResidualBlock(cs[2], cs[2], ks=3, stride=1, dilation=1),
        )

        self.stage3 = nn.Sequential(
            ResidualBlock(cs[2], cs[3], ks=3, stride=2, dilation=1),
            ResidualBlock(cs[3], cs[3], ks=3, stride=1, dilation=1),
        )

        self.stage4 = nn.Sequential(
            ResidualBlock(cs[3], cs[4], ks=3, stride=2, dilation=1),
            ResidualBlock(cs[4], cs[4], ks=3, stride=1, dilation=1),
        )

        self.up1 = nn.ModuleList([
            BasicDeconvolutionBlock(cs[4], cs[5], ks=3, stride=2),
            nn.Sequential(
        ...
        self.up2 = nn.ModuleList([
            BasicDeconvolutionBlock(cs[5], cs[6], ks=3, stride=2),
            nn.Sequential(
        ...
        self.up3 = nn.ModuleList([
            BasicDeconvolutionBlock(cs[6], cs[7], ks=3, stride=2),
            nn.Sequential(
